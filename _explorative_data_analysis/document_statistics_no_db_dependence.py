...
def main():
    ...
    # 4. Build list of RmsId for further merging
    ...
    # 5. Obtain IssuerData (without SQL)
    # Since we don't have access to the database or capfourpy, we will try to load issuer data from a CSV.
    # If the CSV file does not exist, we create a dummy DataFrame.
    issuer_csv_path = os.path.join(project_root, "data", "issuer_data.csv")
    if os.path.exists(issuer_csv_path):
        try:
            data_database = pd.read_csv(issuer_csv_path, dtype=str)
        except Exception as e:
            print(f"[WARNING] Could not read {issuer_csv_path}: {e}")
            data_database = pd.DataFrame(columns=["RmsId", "AbbrevName", "CapFourIndustry", "CountryIsoOperating"])
    else:
        print(f"[WARNING] Issuer data CSV file not found at {issuer_csv_path}. Creating dummy issuer data.")
        data_database = pd.DataFrame({
            "RmsId": list_of_rms_id,
            "AbbrevName": ["Unknown"] * len(list_of_rms_id),
            "CapFourIndustry": ["Unknown"] * len(list_of_rms_id),
            "CountryIsoOperating": ["Unknown"] * len(list_of_rms_id)
        })
    ...
    # Read the two CSVs:
    fs_path = os.path.join(project_root, 'data', 'rms_with_fundamental_score.csv')
    usc_path = os.path.join(project_root, 'data', 'unique_score_combinations.csv')
    ...
    def get_fuzzy_label(row, ref_df, threshold=90):
        ...
        # Use fuzzy matching; extractOne returns (match, score)
