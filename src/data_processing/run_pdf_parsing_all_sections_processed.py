...
def process_prospectus_md(md_file_path,
    ...
    try:
        ...
        # Determine final status for clarity (though we are in as_expected folder)
...
def main():
    ...
    if os.path.exists(data_file):
        try:
            ...
            print(f"{data_file} is empty. Initializing new data.")
            all_data = []
            processed_prospectus_ids = set()
    else:
        print(f"{data_file} is missing. Initializing new data.")
        all_data = []
        processed_prospectus_ids = set()
    ...
    # 5) pdf_to_prospectus_id (we’ll reuse it, though we’re dealing with .md)
    ...
    for RmsId in tqdm(unique_rms_ids, desc="Processing RMS IDs"):
        ...
        for md_filename in md_files:
            ...
            # Check if we already processed this .md
            ...
            try:
                data, next_section_id, status, md_text = process_prospectus_md(
                    ...
                    from_folder='as_expected',  # we know it's from that folder
                    f_year=f_year
                )

                # Add the PDF page count to each row:
                for row in data:
                    row["PDF Page Count"] = pdf_page_count
                    
                # Append to our master data
                ...
                print(f"Processed {md_file_path} with prospectus_id {prospectus_id}")
